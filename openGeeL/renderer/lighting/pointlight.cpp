...
namespace geeL {
	...
	PointLight::PointLight(vec3 position, vec3 diffuse, vec3 specular, vec3 ambient, 
	...
	void PointLight::computeLightTransform() {
		...
		mat4 view = glm::lookAt(position, position + vec3(1.f, 0.f, 0.f), vec3(0.f, -1.f, 0.f));
		lightTransforms.push_back(projection * view);

		view = glm::lookAt(position, position + vec3(-1.f, 0.f, 0.f), vec3(0.f, -1.f, 0.f));
		lightTransforms.push_back(projection * view);

		view = glm::lookAt(position, position + vec3(0.f, 1.f, 0.f), vec3(0.f, 0.f, 1.f));
		lightTransforms.push_back(projection * view);

		view = glm::lookAt(position, position + vec3(0.f, -1.f, 0.f), vec3(0.f, 0.f, -1.f));
		lightTransforms.push_back(projection * view);

		view = glm::lookAt(position, position + vec3(0.f, 0.f, 1.f), vec3(0.f, -1.f, 0.f));
		lightTransforms.push_back(projection * view);
