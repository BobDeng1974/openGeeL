...
vec3 calculateSpecularLighting(vec3 normal, vec3 lightDirection, vec3 viewDirection, float specularMat, vec3 specularLight, bool blinn) {
	...
	//Phong shading
		...
		spec = pow(max(dot(-viewDirection, reflectionDirection), 0.f), 32.f);
	}
...
vec3 calculateSpotLight(int index, SpotLight light, vec3 normal, 
	...
	float shadow = 1.f - calculateSpotLightShadows(index, normal, fragPosition);
...
float calculateSpotLightShadows(int i, vec3 norm, vec3 fragPosition) {
	vec4 posLightSpace = spotLights[i].lightTransform * inverseView * vec4(fragPosition, 1.f);
	...
	float shadow = 0.f;
	vec2 texelSize = 1.f / textureSize(spotLights[i].shadowMap, 0);
...
float calculateDirectionalLightShadows(int i, vec3 norm, vec3 fragPosition) {
	vec4 posLightSpace = directionalLights[i].lightTransform * inverseView * vec4(fragPosition, 1.f);
	vec3 coords = posLightSpace.xyz / posLightSpace.w;
	coords = coords * 0.5f + 0.5f;
