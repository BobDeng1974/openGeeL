...
void main() {
	...
	vec3 span2 = vPosiiton[2] - vPosition[0];
	...
	vec4 position[3];
	pos[0] = transform * gl_in[0].gl_Position;
	pos[1] = transform * gl_in[1].gl_Position;
	pos[2] = transform * gl_in[2].gl_Position;
	...
	vec4 gAABB;
	gAABB.xy = min(pos[0].xy, min(pos[1].xy, pos[2].xy));
	gAABB.zw = max(pos[0].xy, max(pos[1].xy, pos[2].xy));
	...
	vec3 e0 = vec3(pos[1].xy - pos[0].xy, 0.f);
	vec3 e1 = vec3(pos[2].xy - pos[1].xy, 0.f);
	vec3 e2 = vec3(pos[0].xy - pos[2].xy, 0.f);
	...
	//Write vertices
	gl_Position = pos[0];
	fragPosition = pos[0].xyz;
	normal = vNormal[0];
	texCoords = vTexCoords[0];
	EmitVertex();

	gl_Position = pos[1];
	fragPosition = pos[1].xyz;
	normal = vNormal[1];
	texCoords = vTexCoords[1];
	EmitVertex();

	gl_Position = pos[2];
	fragPosition = pos[2].xyz;
	normal = vNormal[2];
