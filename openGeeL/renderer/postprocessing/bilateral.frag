...
float bilateralCoeffient(vec3 baseColor, vec3 sampleColor) {
	...
	float fac = 0.005; //sigma

	return exp(-(diff * diff) / (2 * fac * fac));
...
void main() {             
	...
	for(int i = 1; i < 2; i++) {
		...
		float inBorders = 1.f;//step(TexCoords.x + off.x, 1.f) * 
			//step(0.f, TexCoords.x - off.x) * 
			//step(TexCoords.y + off.y, 1.f) * 
			//step(0.f, TexCoords.y - off.y);

		vec3 samp = texture(image, TexCoords + off).rgb;
		float weight = bilateralCoeffient(base, samp);// * kernel[i];
		weights += weight;

		//Sample right / top pixel
		result += inBorders * samp * weight;

		samp = texture(image, TexCoords - off).rgb;
		weight = bilateralCoeffient(base, samp);// * kernel[i];
		weights += weight;

		//Sample left / bottom pixel
		result += inBorders * samp * weight;
		result = samp * vec3(bilateralCoeffient(base, samp));
    }
