...
namespace geeL {
	...
	void LightSnippet::draw(GUIContext* context) {
		...
		if (nk_tree_push(context, NK_TREE_NODE, id.c_str(), NK_MINIMIZED)) {
			...
			float intensity = GUISnippets::drawBarFloatLogarithmic(context, light.getIntensity(), 1.f, 2000.f, 0.01f, "Intensity");
	...
	void ShadowMapSnippet::draw(GUIContext* context) {
		...
		if (type != ShadowMapType::None) {
			...
			if (type == ShadowMapType::Soft) {
				...
				float scale = GUISnippets::drawBarFloat(context, map.getSoftShadowScale(), 0.f, 100.f, 0.1f, "Scale");
